...
export default function Bubbles() {
  ...
  useEffect(() => {
    ...
    function mk(kind: "normal" | "gold" | "bomb" = "normal") {
      ...
      Object.assign(b.style, {
        ...
        width: size + "px",
        height: size + "px",
        left: Math.random() * (area.clientWidth - size) + "px",
        top: area.clientHeight + "px",
      ...
      b.onclick = () => {
        const gain = kind === "gold" ? 5 : kind === "bomb" ? -3 : 1;
        setScore((s) => Math.max(0, s + gain * Math.max(1, combo)));
        if (kind === "bomb") {
          setCombo(0);
          setHitBomb(true);
          window.setTimeout(() => setHitBomb(false), 400);
        } else {
          setCombo((c) => c + 1);
    ...
    function raf() {
      ...
      listRef.current.forEach((it) => {
        const t = Math.min(1, (now - it.born) / it.dur),
          y = (1 - t) * (H + 60) - 20,
          drift = Math.sin(it.id * 13 + now * 0.0015 + t * 6) * 20;
        it.el.style.transform = `translate(${drift}px, ${-y}px)`;
      });
    ...
    return () => {
      ...
      listRef.current.forEach((it) => it.el.remove());
      listRef.current = [];
      if (comboTimerRef.current) window.clearTimeout(comboTimerRef.current);
    };
  }, []);
  return (
    <div className="container">
      <h1>🫧 戳泡泡</h1>
      <p className="desc">普通/金色/炸弹泡泡，带连击。</p>
      <div
        ref={areaRef}
        className="stage"
        style={{
      ...
      >
        <div className={`badge${hitBomb ? " negative" : ""}`}>得分 {score}</div>
        {combo > 0 && <div className="badge">连击 x{Math.max(1, combo)}</div>}
