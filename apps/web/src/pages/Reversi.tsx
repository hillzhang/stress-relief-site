...
const BAD_X: Pos[] = [{r:1,c:1},{r:1,c:6},{r:6,c:1},{r:6,c:6}]
...
export default function Reversi(){
  ...
  const [aiColor, setAiColor] = useState<Cell>(2) // 电脑执白
  ...
  const place = (r:number, c:number) => {
    ...
    if (theirMoves.length===0 && legalMoves(nb, turn).length===0){
      setBoard(nb); setLast({r,c}); setTurn(nextTurn); return
    }
    if (theirMoves.length===0){
      setBoard(nb); setLast({r,c}); setTurn(turn); return
    }
  ...
  useEffect(()=>{
    if (!vsAI) return
    const empty = score.black + score.white === 4
  ...
  return (
      <>
        ...
        .board-card.theme-light{ --card-bg:#fbfcfe; --card-border:#e6ecf2; --cell-bg:#f7f9fc; --cell-border:#dbe2ea; --accent:#0ea5e9; }
        ...
        @media (max-width: 640px){ .chip{ min-width:120px; } }
        ...
        #board-wrap .overlay .panel{
          min-width: 220px;
          padding: 18px 20px;
          background: #fff;
          border: 1px solid #e2e8f0;
          border-radius: 14px;
          box-shadow: 0 14px 36px rgba(2,6,23,.20);
          text-align: center;
        }
        #board-wrap .overlay .result-title{ font-size:18px; font-weight:800; margin-bottom:4px; }
        #board-wrap .overlay .result-sub{ font-size:14px; color:#64748b; margin-bottom:12px; }
        #board-wrap .overlay .overlay-actions{ display:flex; gap:10px; justify-content:center; }
      `}</style>
      </>
  )
}